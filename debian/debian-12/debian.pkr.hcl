...
variable "iso_file" {
  type    = string
  default = "local:iso/debian-12.12.0-amd64-netinst.iso"
}
...
source "proxmox-iso" "debian_12" {
  ...
  vm_id                 = 9005
  vm_name               = "debian-12-clean-template"
  template_name         = "debian-12-clean-template"
  template_description  = "Clean Debian 12 template: ${var.ssh_username} user, SSH key, qemu guest agent, common shell aliases, Tsinghua mirror."
...
build {
  sources = ["source.proxmox-iso.debian_12"]
}
